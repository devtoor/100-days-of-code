...
addresses = [address.get_text().split(" | ")[-1] for address in soup.select(".list-card-info address")]

prices = [price.contents[0] for price in soup.select(".list-card-price")]

driver = webdriver.Chrome(service=Service(env.get("CHROME_DRIVER_PATH")))
driver.get("https://forms.gle/9aAMbYttoyP86JMB9")
sleep(3)
for n in range(len(links)):
    sleep(2)

    driver.find_element(by=By.XPATH, value='//*[@id="mG61Hd"]/div[2]/div/div[2]/div[1]/div/div/div[2]/div/div['
                                           '1]/div/div[1]/input').send_keys(addresses[n])

    driver.find_element(by=By.XPATH, value='//*[@id="mG61Hd"]/div[2]/div/div[2]/div[2]/div/div/div[2]/div/div['
                                           '1]/div/div[1]/input').send_keys(prices[n])

    driver.find_element(by=By.XPATH, value='//*[@id="mG61Hd"]/div[2]/div/div[2]/div[3]/div/div/div[2]/div/div['
                                           '1]/div/div[1]/input').send_keys(links[n])
