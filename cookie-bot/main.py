...
while True:
    ...
    if time.time() > timeout:
        prices = [price.text.split("-")[1].strip().replace(",", "")
                  for price in driver.find_elements(by=By.CSS_SELECTOR, value="#store b") if price.text]
        cookie_upgrades = {int(prices[n]): item_ids[n] for n in range(len(prices))}
        money = int("".join([char for char in driver.find_element(by=By.ID, value="money").text if char != ","]))
        affordable_upgrades = {cost: item_id for cost, item_id in cookie_upgrades.items() if money > cost}
